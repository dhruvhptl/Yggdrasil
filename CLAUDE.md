...
**Infrastructure:** Background async job queue (orchestrator.rs, tokio mpsc, cap 64). 11 read-model commands (`read_models.rs`). Shared `reqwest::Client` as Tauri state. Prompt/model version logging (`prompt_logs`). Retrieval logging (`mimir_retrieval_logs`). All migrations (001–060) auto-run on startup.
...
src-tauri/migrations/   # 001–060, auto-run on startup
...
### Add a Migration
`src-tauri/migrations/NNN_description.sql` — auto-runs on startup. Never modify existing. Current highest: **060**.
