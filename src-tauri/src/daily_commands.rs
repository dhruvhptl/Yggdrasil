...
pub async fn get_daily_log(
    ...
    // Ensure daily_logs row exists (upsert with no notes)
    sqlx::query(
    ...
    // Fetch notes
    let log_row = sqlx::query("SELECT notes FROM daily_logs WHERE date = $1::date")
        .bind(&date)
        .fetch_one(&database.pool)
        .await
        .map_err(|e| e.to_string())?;
    let notes: Option<String> = log_row.try_get("notes").map_err(|e| e.to_string())?;

    // Fetch links with joined node data
    let link_rows = sqlx::query(
        "SELECT dql.id, dql.date, dql.node_id, dql.free_text, dql.quadrant, \
                dql.sort_order, dql.added_at, \
...
pub async fn add_quest_to_day(
    ...
    let row = sqlx::query(
        "SELECT dql.id, dql.date, dql.node_id, dql.free_text, dql.quadrant, \
         ...
         FROM daily_quest_links dql \
         LEFT JOIN tree_nodes tn ON tn.id = dql.node_id \
         WHERE dql.date = $2::date AND dql.node_id = $3"
    )
    .bind(&id)
    .bind(&date)
    .bind(&node_id)
